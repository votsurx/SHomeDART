...
class _NvrCameraCardState extends ConsumerState<NvrCameraCard> {
  ...
  void _openNvrSettings() async {
    final settings = ref.read(nvrSettingsProvider);
    final url = 'http://${settings.host}:${settings.port}/cameras/$_nvrId/edit';
    try {
      ...
      if (await canLaunchUrl(uri)) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('❌ Не удалось открыть настройки камеры')),
          );
        }
      }
    } catch (e) {
